...
score[2]=0;
ave=0;}
student::student(string a,long long b){
	name=a;
	number=b;
	//score[0]=c;
	//score[1]=d;
...
void student::printList(){
	cout<<name<<"\t"<<number<<"\t"<<endl;//<<score[0]<<"\t"<<score[1]<<"\t"<<score[2]<<"\t"<<ave<<"\t"<<order<<endl;
}
...
void student_list::add(){
		...
		long long n;
		double math,computer,english;
		cout<<"请依次输入学生的姓名，学号（以空格键为分隔符),输入多组数据后键入“end”结束输入"<<endl;
...
switch(t){
	case 1:
		cout<<"姓名"<<"\t"<<"学号"<<endl;//<<"\t"<<"数学"<<"\t"<<"英语"<<"\t"<<"c++"<<"\t"<<"平均分"<<"\t"<<"名次"<<endl;
...
void student_list::menu(){
	...
	int a;
	while(t){
	cout<<"			******************welcome to 日历法管理系统****************"<<endl;
	cout<<"			   	          选择你想进行的操作"<<endl;
	cout<<"			              1.       添加学生数据 "<<endl;
	//cout<<"   	           2.       修改学生数据 "<<endl;
	cout<<"			              2.       删除学生数据 "<<endl;
	cout<<"			              3.       查询学生数据 "<<endl;
	cout<<"			              4.       打印全部学生数据    "<<endl;
	cout<<"			              5.       输出数据项总数"<<endl;
	cout<<"			              6.       退出日历法管理系统"<<endl;
	cout<<"			**********************************************************"<<endl;
	cout<<"      	                by liuzs"<<endl;
		cin>>a;
		switch(a){
			...
			case 5: system("Cls");cout<<"共录入了"<<sum<<"项数据"<<endl;system("pause");system("Cls");break;
			case 6: t=0;
		}
	}
}
