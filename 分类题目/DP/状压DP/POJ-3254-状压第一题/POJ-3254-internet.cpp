...
int dp[20][600];
//dp[i][j]:对于前i行数据，每行有前j种可能状态时的解,以行为单位存储状态
int cur[20];
//cur[i]表示的是第i行整行的情况，输入时就要按位来存储，cur[i]表示的是第i行整行的情况，每次改变该数字的二进制表示的一位

inline bool ok(int x)   //判断状态x是否可行
{
    if(x&x<<1)  return false;//若存在相邻两个格子都为1，则该状态不可行，利用位运算计算是否相邻
    return true;
}
void init()       //遍历所有可能的状态,line47使用
{
    top = 0;
...
int main()
{
    ...
    while(scanf("%d%d",&M,&N)!= EOF) {
        ...
        for(int i = 1; i <= top; i++) {
            if(fit(state[i],1)) { //判断所有可能状态与第一行的实际状态的逆是否有“重合”;有哪些状态符合第一行的实际状态
